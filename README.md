...
1. rancher-aws-k3s-rke 폴더 하위 tfvars 내 AWS 크레덴셜 기재함으로 local-backend 소스로만 사용해야 함
2. GITOPS 용도로 생성된 소스가 아니며 부득이하게 git 에 올려 gitops 방식으로 사용 할 경우 AWS 프로파일 임포트 방식으로 소스변경 필요
3. Terraform 클라우드를 백엔드 사용시, 테라폼 클라우드 용도로 소스 일부 수정 후  AWS 크레덴셜 키 변수 선언만 하고 키 값은 UI 에 입력하여 사용

```


# AWS Rancher Quickstart-AWS
